...
unsigned sparse_agg_xor_1mb_prefetch(const unsigned* src) {
  unsigned res;
  for (int i = 0; i < 1024*1024/4; i += 64) {
    asm("prefetch.r 256(%0)" :: "r" (src+i));
    res ^= src[i];
    asm("prefetch.r 320(%0)" :: "r" (src+i));
    res ^= src[i + 16];
    asm("prefetch.r 384(%0)" :: "r" (src+i));
    res ^= src[i + 32];
    asm("prefetch.r 448(%0)" :: "r" (src+i));
    res ^= src[i + 48];
  }
  return res;
}
...
void sparse_inplace_xor_1mb_prefetch(unsigned* data, unsigned v) {
  for (int i = 0; i < 1024*1024/4; i += 64) {
    asm("prefetch.r 256(%0)" :: "r" (data+i));
    data[i] ^= v;
    asm("prefetch.r 320(%0)" :: "r" (data+i));
    data[i + 16] ^= v;
    asm("prefetch.r 384(%0)" :: "r" (data+i));
    data[i + 32] ^= v;
    asm("prefetch.r 448(%0)" :: "r" (data+i));
    data[i + 48] ^= v;
  }
}
